...
import { exportAdminUsers, getAdminExhibitorStats, getAdminUsers } from '../../api/identity';
...
function AdminExhibitorList() {
  ...
  // 통계 카드는 검색/필터와 무관하게 참가업체 전체 기준 - 목록 필터가 바뀌어도 다시 불러오지 않는다.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    getAdminUsers({
      ...
      .catch((err) => setLoadError(err.response?.data?.error?.message ?? '참가업체 목록을 불러오지 못했습니다.'));
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = match ? decodeURIComponent(match[1]) : `exhibitors_${toDateParam(new Date())}.csv`;
