# 장남장녀단 — 자동차 박람회 예약 관리 플랫폼

멋사 백엔드 24기 심화 프로젝트 — Agile + MSA로 4주간(2026.08.31\~09.30) 진행하는 자동차 박람회 예약 관리 플랫폼입니다.

## 진입점

* Notion : https://app.notion.com/p/4-3c973873401a804da04afc2abb3c357a
* GitHub Project : https://github.com/likelion-backend-24th/Final-Project-Team4
* Issue 템플릿: [.github/ISSUE\_TEMPLATE](.github/ISSUE_TEMPLATE)
* 문서 인덱스 (Notion 원본 링크 목록): [docs/README.md](docs/README.md)

## 팀 구성

|이름|역할|
|-|-|
|이동건|팀장|
|김다솜|팀원|
|김재혁|팀원|
|정의찬|팀원|

## 실행 방법

TODO

